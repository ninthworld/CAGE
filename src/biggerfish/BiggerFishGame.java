...
//        cameraController.setMaxRadius(64.0f);
        cameraController.setMaxRadius(512.0f);
        ...
        engine.getInputManager().addAction(keyboard, Key.F5, InputActionType.PRESS, (deltaTime, event) -> { panCamera = !panCamera; });
        engine.getInputManager().addAction(keyboard, Key.F6, InputActionType.REPEAT, ((deltaTime, event) -> cameraController.setAzimuth(cameraController.getAzimuth() + deltaTime * 0.5f)));
        engine.getInputManager().addAction(keyboard, Key.PAGEUP, InputActionType.PRESS, (deltaTime, event) -> player.setMass(player.getMass() * 2.0f));
        engine.getInputManager().addAction(keyboard, Key.PAGEDOWN, InputActionType.PRESS, (deltaTime, event) -> player.setMass(player.getMass() * 0.5f));
    }
