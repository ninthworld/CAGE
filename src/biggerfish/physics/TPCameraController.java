...
public class TPCameraController extends NodeController {
    ...
    public void updateNode(float deltaTime, Node node) {
        if(updated) {
            final float linearMul = 500.0f;
            final float angularMul = -80.0f;
            node.moveForward(forward * linearMul);
            node.moveRight(right * linearMul);
            node.translate(Direction.UP.mul(up * linearMul, new Vector3f()));
            node.rotate(yaw * angularMul, node.getLocalRotation().invert(new Matrix3f()).getColumn(1, new Vector3f()));
            ...
            if((pitch < 0.0f && pitchAngle < maxPitch / Math.PI * 2.0f) || (pitch > 0.0f && pitchAngle > minPitch / Math.PI * 2.0f)) {
                node.rotate(pitch * angularMul, Direction.RIGHT);
            }
        }
